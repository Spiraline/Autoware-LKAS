...
p1 = sim.add_agent("Bob", lgsvl.AgentType.PEDESTRIAN, state)


#set npc vehicle2's initial position
state = lgsvl.AgentState()
state.transform.position = spawns[0].position + 70 * forward + 2 * right
state.transform.rotation = spawns[0].rotation

p2 = sim.add_agent("Bob", lgsvl.AgentType.PEDESTRIAN, state)
...
p3 = sim.add_agent("Bob", lgsvl.AgentType.PEDESTRIAN, state)
